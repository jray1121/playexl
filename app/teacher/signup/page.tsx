...
export default function TeacherSignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { name, school, role: "teacher" } },
    })
    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8 gap-2">
          ...
          <h1 className="text-2xl font-bold text-zinc-100">Create Teacher Account</h1>
          <p className="text-zinc-400 text-sm">Purchase licenses and manage your classes</p>
        </div>
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">School / Organization</label>
        ...
        <p className="text-center text-zinc-400 text-sm mt-6">
          ...
          <Link href="/teacher/login" className="text-brand hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
